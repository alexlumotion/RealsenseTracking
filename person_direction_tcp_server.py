# person_direction_tcp_server.py
import cv2
import numpy as np
...
try:
    while True:
        ...
        if person_present:
            ...
            if cooldown > 0:
                ...
                if len(xs) >= 2:
                    ...
                    if moved_right:
                        emit("MOVE_RIGHT", {"x": ema_x, "speed": speed})
                        cooldown = COOLDOWN_FRAMES
                        xs.clear()
                    elif moved_left:
                        emit("MOVE_LEFT", {"x": ema_x, "speed": speed})
                        cooldown = COOLDOWN_FRAMES
                        xs.clear()
        ...
        cv2.imshow("TCP server | Direction only (color | depth)", view)
