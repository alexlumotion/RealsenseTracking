# person_direction_tcp_local_debug.py
import cv2, numpy as np, pyrealsense2 as rs, mediapipe as mp
from collections import deque
import socket, threading, time, csv, os, json
    ...
    def _accept_loop(self):
        while self.running:
            try:
                ...
                with self.lock:
                    self.clients.add(c)
                print(f"[TCP] Client connected: {a}")
                threading.Thread(target=self._client_keepalive, args=(c, a), daemon=True).start()
            except OSError:
                break

    def _client_keepalive(self, c, a):
        try:
            while self.running:
                # легкий keepalive/перевірка "живий чи ні"
                try:
                    peek = c.recv(1, socket.MSG_PEEK)
                    if not peek:
                        break
            ...
            try:
                ...
                pass
            print(f"[TCP] Client disconnected: {a}")
...
tcp = TcpBroadcaster("127.0.0.1", 5050)
tcp.start()
...
def send_event(event: str, **payload):
    ...
    tcp.send(msg)
    print(f"[TCP] {event}  extra={payload}")
...
try:
    while True:
        ...
        # дзеркальний фліп і для RGB, і для depth
        ...
        cv2.imshow("TCP local | Direction with debug", view)
